...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('absensi.store') }}" id="formAbsensi">
                ...
                <div>
                    ...
                    <select name="tahun_pelajaran_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm">
                        ...
                        @foreach($tahunPelajarans as $t)
                            <option value="{{ $t->id }}" @selected(old('tahun_pelajaran_id')==$t->id)>{{ $t->nama_tahun }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="semester_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm">
                        ...
                        @foreach($semesters as $s)
                            <option value="{{ $s->id }}" @selected(old('semester_id')==$s->id)>{{ $s->nama_semester }}</option>
                        @endforeach
                    </select>
                </div>
            </div>

            <div id="siswaContainer" class="{{ old('absensi') ? '' : 'hidden' }}">
                ...
                <div class="flex items-center gap-3 mt-6" id="submitContainer" style="display:none">
                    <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
                        <i class="ph ph-check"></i> Simpan Semua Absensi
                    </button>
            ...
            <div id="emptyKelas" class="text-center py-8 text-gray-400 {{ old('absensi') ? 'hidden' : '' }}">
...
document.getElementById('kelas_id').addEventListener('change', function() {
    ...
    fetch('/admin/get-siswa-by-kelas/' + kelasId)
        .then(res => res.json())
        .then(siswas => {
                ...
                html += `
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 sm:px-6 py-3 text-center">
                            <input type="number" name="absensi[${s.id}][sakit]" value="0" min="0" class="w-20 text-center px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none mx-auto block">
                        </td>
                        <td class="px-4 sm:px-6 py-3 text-center">
                            <input type="number" name="absensi[${s.id}][izin]" value="0" min="0" class="w-20 text-center px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none mx-auto block">
                        </td>
                        <td class="px-4 sm:px-6 py-3 text-center">
                            <input type="number" name="absensi[${s.id}][alfa]" value="0" min="0" class="w-20 text-center px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none mx-auto block">
                        </td>
                    </tr>
                `;
            });
