        ...
        <form method="POST" action="{{ route('kelas.update', $kelas) }}">
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm {{ $errors->has('nama_kelas') ? 'border-red-500' : '' }}">
                ...
                <div>
                    ...
                    <select name="tingkat" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm {{ $errors->has('tingkat') ? 'border-red-500' : '' }}">
                        ...
                        @for($i=1; $i<=6; $i++)
                            <option value="{{ $i }}" @selected(old('tingkat', $kelas->tingkat)==$i)>{{ $i }}</option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <select name="wali_kelas_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm {{ $errors->has('wali_kelas_id') ? 'border-red-500' : '' }}">
                        <option value="">Pilih...</option>
                        @foreach($gurus as $g)
                            <option value="{{ $g->id }}" @selected(old('wali_kelas_id', $kelas->wali_kelas_id)==$g->id)>{{ $g->nama_guru }}</option>
                        @endforeach
                    </select>
