...
@section('content')
<div class="p-6">
    <h1 class="text-2xl font-bold mb-6">Edit Semester</h1>
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <form method="POST" action="{{ route('semester.update', $semester) }}">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <select name="nama_semester" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm">
                        <option value="Ganjil" @selected(old('nama_semester', $semester->nama_semester)=='Ganjil')>Ganjil</option>
                        <option value="Genap" @selected(old('nama_semester', $semester->nama_semester)=='Genap')>Genap</option>
                    </select>
                </div>
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="status_aktif" value="1" @checked(old('status_aktif', $semester->status_aktif)) class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
